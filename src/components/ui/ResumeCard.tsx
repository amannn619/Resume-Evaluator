...
interface ResumeCardProps {
    ...
    onReupload?: (resume: Resume) => void
    onDelete?: (resume: Resume) => void
}
export default function ResumeCard({ resume, onDownload, onReupload, onDelete }: ResumeCardProps) {
    ...
    return (
        <div className="bg-surface border border-outline rounded-xl p-4 flex items-center justify-between hover:border-brand transition-colors group">
            ...
            <div className="flex items-center gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity shrink-0">
                ...
                <button
                    onClick={() => onReupload?.(resume)}
                    className="p-2 text-subtle hover:text-brand hover:bg-background rounded-md transition-colors"
                    title="Re-upload"
                >
                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
